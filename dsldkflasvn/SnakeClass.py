...
#Revival System Add-On
death_count = 0          # how many times player has died
base_chance = 50         # starting revival chance
chance_loss = 5         # percent lost after every death

def try_revive():
    """
    Returns True if the revival succeeds, False otherwise.
    Chance = 25% - (death_count * 6%), minimum 0%
    """
    global death_count, base_chance, chance_loss

    revive_chance = max(0, base_chance - (death_count * chance_loss))
    roll = random.randint(1, 100)
    print(f"Revival chance: {revive_chance}% | Roll: {roll}")

    success = roll <= revive_chance
    death_count += 1

    return success, revive_chance
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if game_over:
            ...
            # revival failed → normal game over
            game.game_over_menu()
